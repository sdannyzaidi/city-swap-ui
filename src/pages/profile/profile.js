...
const Profile = () => {
	...
	const handleSubscription = () => {
		...
		fetch(`${process.env.REACT_APP_BACKEND_BASE_URL}${endpoints['create-subscription']}`, {
			...
			body: JSON.stringify({ email: loggedInUser?.email, renew: true }),
		}).then((response) => {
			if (response.status === 200) {
				...
				notification.error({ message: 'Error', description: 'Unable to subscribe and proceed further. Please try again later.' })
				setButtonLoading(false)
			}
		})
	}
							...
							<div className='flex basis-2/3 '>
								{!loggedInUser?.subscriptionId ? (
									<div className='flex flex-col items-start'>
										<div className='font-medium w-[80%] text-xs mb-4'>You are not subscribed to our monthly subscription please click below if you aim to.</div>
										<Button loading={buttonLoading} onClick={handleSubscription} className='btn-primary w-1/2 mb-5 max-sm:text-xs'>
											Renew Subscription
										</Button>
									</div>
								) : (
									<div className='flex flex-col'>
										...
										<div className='flex flex-row items-center py-4'>
											...
											<Icon
												onClick={() => navigator(`${pathname}/${loggedInUser?._id}/updateSubscription`)}
												path={mdiPencil}
												size={0.8}
