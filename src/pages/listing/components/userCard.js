...
const UserCard = ({ listing, setVisible }) => {
	...
	const freeUntil = loggedInUser ? moment(loggedInUser?.createdAt).add(30, 'days').diff(moment(), 'seconds') : -1

	console.log({ loggedInUser })
	console.log({ freeUntil, subscribed })
		...
		<div className='basis-1/3 px-[27px] py-[22px] w-full rounded-lg border border-solid border-[#F2F4F7] mt-12'>
			<div className='flex flex-col justify-between sm:items-center max-md:items-start'>
				...
				<div className='flex flex-row justify-start items-center w-full space-x-4 pt-8'>
					<Button className='btn-primary' disabled={subscribed ? false : subscribed || freeUntil < 0} onClick={() => setVisible(true)}>
						Request Owner
					</Button>
					<Button
						disabled={subscribed ? false : subscribed || freeUntil < 0}
